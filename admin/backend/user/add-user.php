...
if($_SERVER['REQUEST_METHOD'] === "POST"){
    if (isset($_POST["email"]) && isset($_POST["city"]) && isset($_POST["password"]) && isset($_POST["city"]) && isset($_POST["country"]) && isset($_POST["referral_code"])) {
    ...
    $data->referral_code=$_POST["referral_code"];
        if (empty(searchByEmail($data))) {
            if (createUser($data)) {
                ...
                $_SESSION["message_error"] = "failed to create user";
                header("location:../manage-users.php");
            }
        }else{
